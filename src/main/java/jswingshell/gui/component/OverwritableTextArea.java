...
/**
 * A {@code OverwritableTextArea} is a multi-line area that displays plain text, with overriding
 * mode management.
 * 
 * <p>
 * It is an adaptation of the {@code OverwritableTextField} by Kim Topley from <em>Core SWING
 * Advanced Programming</em>. This version comes with all the constructors from the
 * {@link JTextArea} by reusing its basic caret as the insert caret.
 * </p>
 * 
 * <p>
 * The insert caret is initialized with the {@code JTextArea}'s basic caret. The insert caret can be
 * changed using the {@link #setInsertCaret(javax.swing.text.Caret)} method.
 * </p>
 * 
 * <p>
 * The default overwrite caret is the {@link OverwriteCaret}. The overwrite caret can then be
 * changed through the {@link #setOverwriteCaret(javax.swing.text.Caret)} method. Setting the
 * overwrite or the insert caret will check the current overwrite mode and update the
 * {@code JTextArea} caret used, but it will not shift the overwriting mode. To do so, use the
 * {@link #setOverwriting(boolean)} or the {@code INSERT} key.
 * </p>
 * 
 * <p>
 * Switching from overwrite mode will change the current and use the new one, with all its
 * properties set as the original caret (i.e. dot, mark, blink rate, and listeners).
 * </p>
 * 
 * <p>
 * The {@code JTextArea}'s {@link #setCaret(javax.swing.text.Caret)} method is now used to set the
 * caret for the current mode:
 * </p>
 * <ul>
 * <li>If {@code #isOverwriting()} is {@code true}, the {@code setCaret} will call
 * {@code #setOverwriteCaret}.</li>
 * <li>If {@code #isOverwriting()} is {@code false}, the {@code setCaret} will call
 * {@code #setInsertCaret}.</li>
 * </ul>
 * 
 * @author Kim Topley
...
public class OverwritableTextArea extends JTextArea {
  ...
  /**
   * Constructs a new OverwritableTextArea with the specified text displayed. A default model is
   * created and rows/columns are set to 0.
   *
  ...
  /**
   * Constructs a new empty OverwritableTextArea with the specified number of rows and columns. A
   * default model is created, and the initial string is null.
   *
  ...
  /**
   * Constructs a new OverwritableTextArea with the specified text and number of rows and columns. A
  ...
  /**
   * Constructs a new OverwritableTextArea with the given document model, and defaults for all of
   * the other arguments (null, 0, 0).
   *
  ...
  /**
   * Constructs a new OverwritableTextArea with the specified number of rows and columns, and the
   * given model. All of the constructors feed through this constructor.
   *
  ...
  public void replaceSelection(String content) {
    ...
    if (doc != null) {
      /*
       * If we are not overwriting, just do the usual insert. Also, if there is a selection, just
       * overwrite that (and that only).
       */
      if (overwriting == true && getSelectionStart() == getSelectionEnd()) {

        /*
         * Overwrite and no selection. Remove the stretch that we will overwrite, then use the usual
         * code to insert the new text.
         */
   ...
   *
   * @return {@code true} if the overwrite mode is globally enabled, {@code false} otherwise
   */
  ...
  /**
   * Select the appropriate caret for the current overwrite mode and update used caret if needed.
   */
  ...
  public static final String toggleOverwriteAction =
      "toggle-overwrite-" + OverwritableTextArea.class.toString();
