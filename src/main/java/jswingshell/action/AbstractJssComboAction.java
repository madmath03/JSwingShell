...
/**
 * A {@code AbstractJssComboAction} is a multiple choice operation that can be called through an id
 * and a parameter.
 *
 * <p>
 * This action is used to define a status or mode among multiple choice. It is higly appropriate for
 * {@code JComboBox} GUI components.
 * </p>
 *
 * <p>
 ...
 * @since 1.2
 */
public abstract class AbstractJssComboAction<T> extends AbstractJssAction
  ...
  /**
   * The inner action group that the internal {@link ComboElementAction} belongs to.
   *
   * @since 1.3
   */
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified array of objects to construct a
   * default model for combo boxes.
   *
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified default shell controller.
   *
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified array of objects to construct a
   * default model for combo boxes and default shell controller.
   *
   * @param items a model for combo boxes
   * @param shellController the default shell controller
   */
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified default shell controller and
   * default arguments.
   *
   * @param shellController the default shell controller
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified array of objects to construct a
   * default model for combo boxes, default shell controller and default arguments.
   *
   * @param items a model for combo boxes
   * @param shellController the default shell controller
   * @param args the default arguments for executing this action through a shell
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified name, default shell controller and
   * default arguments.
   *
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified array of objects to construct a
   * default model for combo boxes, name, default shell controller and default arguments.
   *
   * @param items a model for combo boxes
   * @param name the name ({@code Action.NAME}) for the action; a value of {@code null} is ignored
   * @param shellController the default shell controller
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified name, small icon, default shell
  ...
  /**
   * Creates an {@code AbstractJssComboAction} with the specified array of objects to construct a
   * default model for combo boxes, name, small icon, default shell controller and default
   * arguments.
   *
   * @param items a model for combo boxes
   * @param name the name ({@code Action.NAME}) for the action; a value of {@code null} is ignored
  ...
  /**
   * Returns the data model currently used by the {@code AbstractJssComboAction}.
   *
   * @return the {@code ComboBoxModel} that provides the displayed list of items
   */
   ...
   * {@code ListDataListener}s that the contents have changed.
   *
   * @param anItem the list object to select or {@code null} to clear the selection
   */
  @Override
  ...
  public void putValue(String key, Object newValue) {
    ...
    if (oldValue == null || !oldValue.equals(newValue)) {
      ...
      if (SELECTED_KEY.equals(key) && getDefaultArguments() != null
    ...
    if (e != null && e.getSource() instanceof JComboBox) {
      ...
      if (selectedItem != null) {
        ...
        eventArgs = new String[] {commandIdentifier,
            valuesByArgument.get(nextSelectedItem).iterator().next()};
      }
    }
  ...
  public int run(IJssController shellController, String... args) {
    ...
    if (args != null && args.length > 1) {
      ...
      if (args.length == 2 && args[1] != null && argumentsByValue != null) {
        ...
        /*
         * XXX What to do... Here, we are handling the invalid argument received by publishing an
         * error message (which is hard coded by the way). Even if it seems coherent with a
         * "command line only" usage of the shell, it prevents any kind of interaction with the
         * user, thus not allowing him to fix his error... A solution would be to throw an exception
  ...
  private static <T> Map<String, T> constructValuesByArgument(
      Map<T, Collection<String>> argumentsByValue) {
   ...
   * The inner action group that the internal {@link ComboElementAction} belongs to.
   *
   * @return inner action group that the internal {@link ComboElementAction} belongs to.
   *
  ...
  /**
   * Set the inner action group that the internal {@link ComboElementAction} belongs to.
   *
   * @param innerGroup the new inner action group that the internal {@link ComboElementAction}
   *        belongs to.
   *
   * @see #getInnerGroup()
   * @see #getInnerElementActions()
   ...
   *
   * <p>
   * Each {@code ComboElementAction} is a switch action which shall trigger one of this combo action
   * value. The consistency between the switch actions and the combo selected item is managed
   * through an {@link ActionGroup}.
   * </p>
   *
   * <p>
   * Calling this method will initialize the internal combo element actions. If you need to check if
   * the action currently has defined those internal actions, use
   * {@link #hasInnerElementActions() }.
   * </p>
   *
   * <p>
   * If you need to update the model for this combo action, make sure to reset the inner elements
   * actions ({@link #resetInnerElements() }) to reflect the changes on the model.
   * </p>
   *
   * @return internal collection of {@link ComboElementAction}s referencing combo items.
   *
   ...
   *
   * @return {@code true} if the action has any inner switch actions, {@code false} otherwise.
   *
   * @since 1.4
   */
  ...
  /**
   * Initialize internal {@link ComboElementAction}s referencing combo items and the associated
   * internal {@link ActionGroup}.
   *
  ...
  protected Collection<ComboElementAction<T>> initInnerElements() {
    ...
    for (int i = 0, n = dataModel.getSize(); i < n; i++) {
      ComboElementAction<T> elementAction =
          this.new ComboElementAction<>(this, dataModel.getElementAt(i));
  ...
  /**
   * Reset the internal {@link ComboElementAction}s and internal {@link ActionGroup}.
   *
   * @see #initInnerElements()
   * @since 1.3
   */
  ...
  // #########################################################################
  /**
   * A {@code ComboElementAction} is a switch operation (<em>on/off</em>) that can be called through
   * an id and a parameter, that refers to a {@link AbstractJssComboAction} item.
   *
   * <p>
  ...
  public class ComboElementAction<T> extends AbstractJssSwitchAction {
    ...
    /**
     * Construct a {@code ComboElementAction} from a parent {@link AbstractJssComboAction} and one
     * of its associated model's data.
     *
     ...
     *
     * <p>
     * The next call to {@link #getBriefHelp() } will initialize the action's brief help. In the
     * meantime, the brief help will remain empty.
     * </p>
     *
     * @see #getBriefHelp()
     * @see #initBriefHelp()
