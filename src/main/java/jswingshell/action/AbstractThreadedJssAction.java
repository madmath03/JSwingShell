...
/**
 * A {@code AbstractThreadedJssAction} is an operation that can be called through anid and some
 * parameters if needed and executes in a separate thread.
 *
 * @see SwingWorker
 *
...
public abstract class AbstractThreadedJssAction extends AbstractJssAction {
   ...
   *
   * <p>
   * The method will prepare a {@link AbstractJssActionWorker} for the given shell and arguments and
   * if it was properly created, execute the working and return {@link IJssAction#IN_PROGRESS}.
   * </p>
   *
   ...
   *
   * @return an unmodiable list of all currently active workers for this action
   *
   * @since 1.4
   */
  ...
  /**
   * Attempts to cancel execution of this action's tasks. This attempt will fail if the task has
   * already completed, has already been cancelled, or could not be cancelled for some other reason.
   * If successful, and this task has not started when <tt>cancel</tt> is called, this task should
  ...
  // #########################################################################
  /**
   * The type used for carrying out intermediate results by a {@code AbstractJssActionWorker's}
  ...
  /**
   * An abstract class to perform lengthy GUI-interaction action's tasks in a background thread.
   *
   * <p>
   * Several background threads can be used to execute such tasks. However, the exact strategy of
   * choosing a thread for any particular {@code AbstractJssActionWorker} is unspecified and should
   * not be relied on.
   * </p>
   *
   * <p>
   * When writing a multi-threaded application using Swing, there are two constraints to keep in
   * mind: (refer to <a href="http://java.sun.com/docs/books/tutorial/uiswing/misc/threads.html">
   ...
   *
   * <p>
   * {@code AbstractJssActionWorker} is designed for situations where you need to have a long
   * running action run in a background thread and provide updates to the UI either when done, or
   * while processing. Subclasses of {@code AbstractJssActionWorker} must implement the
   * {@link #doInBackground} method to perform the background computation.
   * </p>
   *
   *
   * <p>
   ...
   * </p>
   * <ul>
   * <li><i>Current</i> thread: The {@link #execute} method is called on this thread. It schedules
   * {@code SwingWorker} for the execution on a <i>worker</i> thread and returns immediately. One
   * can wait for the {@code SwingWorker} to complete using the {@link #get get} methods.</li>
   * <li><i>Worker</i> thread: The {@link #doInBackground} method is called on this thread. This is
   * where all background activities should happen. To notify {@code PropertyChangeListeners} about
   * bound properties changes use the {@link #firePropertyChange firePropertyChange} and
   * {@link #getPropertyChangeSupport} methods. By default there are two bound properties available:
   * {@code state} and {@code progress}.</li>
   * <li><i>Event Dispatch Thread</i>: All Swing related activities occur on this thread.
   * {@code SwingWorker} invokes the {@link #process process} and {@link #done} methods and notifies
   * any {@code PropertyChangeListeners} on this thread.</li>
   * </ul>
   *
   * <p>
   ...
   * {@code SwingWorker} notifies any {@code PropertyChangeListeners} about the {@code state}
   * property change to {@code StateValue.STARTED}. After the {@code doInBackground} method is
   * finished the {@code done} method is executed. Then {@code SwingWorker} notifies any
   * {@code PropertyChangeListeners} about the {@code state} property change to
   * {@code StateValue.DONE}.
   * </p>
   *
   * <p>
   * {@code SwingWorker} is only designed to be executed once. Executing a {@code SwingWorker} more
   * than once will not result in invoking the {@code doInBackground} method twice.
   * </p>
   *
   * <p>
   * Because {@code SwingWorker} implements {@code Runnable}, a {@code SwingWorker} can be submitted
  ...
  public abstract class AbstractJssActionWorker
    ...
    protected void process(List<JssActionWorkerChunk> chunks) {
      ...
      /*
       * Due to the asynchronous mode, make sure no message is published when the worker has already
       * ended.
       */
